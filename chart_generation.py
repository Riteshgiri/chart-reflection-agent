"""
M2 Agentic AI - Chart Generation (reflection pattern)

Workflow:
  1) Generate an initial version (V1) of the plotting code with an LLM
  2) Execute the code and create the chart
  3) Reflect on the chart image + code with an LLM
  4) Generate and execute the improved version (V2)
...
# ---------------------------------------------------------------------------
# Step 3 — Reflect on the output and regenerate (V2)
# ---------------------------------------------------------------------------
...
def run_workflow(
    ...
    show(df.sample(n=5), title="Random Sample of Dataset")
    ...
    # 1) Generate code (V1)
    show("Step 1: Generating chart code (V1)… 📈")
    code_v1 = generate_chart_code(
        instruction=user_instructions,
        model=generation_model,
        out_path_v1=out_v1,
    )
    show(code_v1, title="LLM output with first draft code (V1)")

    # 2) Execute V1 (hard-coded: extract <execute_python> block and run immediately)
    show("Step 2: Executing chart code (V1)… 💻")
    match = re.search(r"<execute_python>([\s\S]*?)</execute_python>", code_v1)
    if match:
        initial_code = match.group(1).strip()
        exec_globals = {"df": df}
        exec(initial_code, exec_globals)
    show(f"Saved {out_v1}", title="Generated Chart (V1)")

    # 3) Reflect on V1 (image + original code) to get feedback and refined code (V2)
    show("Step 3: Reflecting on V1 (image + code) and generating improvements… 🔁")
    ...
    show(feedback, title="Reflection feedback on V1")
    show(code_v2, title="LLM output with revised code (V2)")

    # 4) Execute V2 (hard-coded: extract <execute_python> block and run immediately)
    show("Step 4: Executing refined chart code (V2)… 🖼️")
    ...
    show(f"Saved {out_v2}", title="Regenerated Chart (V2)")
...
if __name__ == "__main__":
    ...
    generation_model = "claude-haiku-4-5"   # fast model for code generation (lab default: "gpt-4o-mini")
    reflection_model = "claude-sonnet-4-6"  # stronger model for reflection (lab default: "o4-mini")
